   ...
   */
    public function cols_that_should_remain_ints()
    {
        return [
            ...
            'syncing_totals_orders',
            'syncing_totals_customers',
            'syncing_totals_webhooks',
            'syncing_totals_media',
            'syncing_step_total',
            'syncing_step_current',
            'syncing_current_amounts_shop',
            ...
            'syncing_current_amounts_orders',
            'syncing_current_amounts_customers',
    ...
    public function get_column_defaults()
    {
        return [
            ...
            'syncing_totals_customers' =>
                $this->default_syncing_totals_customers,
            ...
            'syncing_current_amounts_shop' =>
                $this->default_syncing_current_amounts_shop,
            'syncing_current_amounts_smart_collections' =>
            ...
            'syncing_current_amounts_orders' =>
                $this->default_syncing_current_amounts_orders,
            'syncing_current_amounts_customers' =>
                $this->default_syncing_current_amounts_customers,
            'syncing_current_amounts_webhooks' =>
    ...
    public function set_syncing_totals($counts, $exclusions = [])
    {
        ...
        $counts_customers = isset($counts['customers'])
            ? $counts['customers']
            : 0;
        ...
        $collects_totals =
            $exclusions && isset($exclusions['collects'])
                ...
                : $counts_collects;
        $orders_totals =
            $exclusions && isset($exclusions['orders']) ? 0 : $counts_orders;
        $customers_totals =
            $exclusions && isset($exclusions['customers'])
                ? 0
                : $counts_customers;
        $webhooks_totals =
        ...
        return [
            'shop' => $this->update_column_single(
                ['syncing_totals_shop' => $shop_totals],
                ['id' => 1]
            ),
            'smart_collections' => $this->update_column_single(
                [
                    'syncing_totals_smart_collections' => $smart_collections_totals,
                ],
                ['id' => 1]
            ),
            'custom_collections' => $this->update_column_single(
                [
                    'syncing_totals_custom_collections' => $custom_collections_totals,
                ],
                ['id' => 1]
            ),
            'products' => $this->update_column_single(
                ['syncing_totals_products' => $products_totals],
                ['id' => 1]
            ),
            'collects' => $this->update_column_single(
                ['syncing_totals_collects' => $collects_totals],
                ['id' => 1]
            ),
        ];
    }
    ...
    public function reset_syncing_totals()
    {
        return [
            'shop' => $this->update_column_single(
                ['syncing_totals_shop' => 0],
                ['id' => 1]
            ),
            'smart_collections' => $this->update_column_single(
                ['syncing_totals_smart_collections' => 0],
                ['id' => 1]
            ),
            'custom_collections' => $this->update_column_single(
                ['syncing_totals_custom_collections' => 0],
                ['id' => 1]
            ),
            'products' => $this->update_column_single(
                ['syncing_totals_products' => 0],
                ['id' => 1]
            ),
            'collects' => $this->update_column_single(
                ['syncing_totals_collects' => 0],
                ['id' => 1]
            ),
        ];
    }
    ...
    {
        return [
            'syncing_start_time' => $this->update_column_single(
                ['syncing_start_time' => 0],
                ['id' => 1]
            ),
            'syncing_end_time' => $this->update_column_single(
                ['syncing_end_time' => 0],
                ['id' => 1]
            ),
        ];
    ...
    /*
     ...
     Reset syncing published product ids

     */
    public function reset_syncing_published_product_ids()
    {
        return $this->update_column_single(
    ...
    {
        return [
            'syncing_errors' => $this->update_column_single(
                ['syncing_errors' => false],
                ['id' => 1]
            ),
            'syncing_warnings' => $this->update_column_single(
                ['syncing_warnings' => false],
                ['id' => 1]
            ),
        ];
    }
    ...
    public function reset_syncing_current_amounts()
    {
        return [
            'shop' => $this->update_column_single(
                ['syncing_current_amounts_shop' => 0],
                ['id' => 1]
            ),
            'smart_collections' => $this->update_column_single(
                ['syncing_current_amounts_smart_collections' => 0],
                ['id' => 1]
            ),
            'custom_collections' => $this->update_column_single(
                ['syncing_current_amounts_custom_collections' => 0],
                ['id' => 1]
            ),
            'products' => $this->update_column_single(
                ['syncing_current_amounts_products' => 0],
                ['id' => 1]
            ),
            'collects' => $this->update_column_single(
                ['syncing_current_amounts_collects' => 0],
                ['id' => 1]
            ),
        ];
    }
    ...
    public function increment_current_amount($key, $amount_to_increment = false)
    {
        ...
        $query =
            'UPDATE ' .
            $this->table_name .
            ' SET syncing_current_amounts_' .
            $key .
            ' = syncing_current_amounts_' .
            $key .
            ' + ' .
            $amount_to_increment;
    ...
    {
        $current_chunk_string = $this->get_col_value(
            'recently_syncd_media_ref',
            'string'
        );
    ...
    public function update_recently_syncd_media_ref($media_chunk)
    {
        ...
        return $this->update_column_single(
            ['recently_syncd_media_ref' => maybe_serialize($current_chunk)],
            ['id' => 1]
        );
    }
    ...
    public function set_published_product_ids($published_product_ids)
    {
        ...
        return $this->update_column_single(
            [
                'published_product_ids' => maybe_serialize(
                    $published_product_ids
                ),
            ],
            ['id' => 1]
    ...
    public function expire_sync()
    {
        return [
            ...
            'delete_cache' => Transients::delete_cache(),
        ];
    }
    ...
    public function save_error($error_message)
    {
        ...
        $saved_Results = $this->update_column_single(
            ['syncing_errors' => $serialized_errors],
            ['id' => 1]
        );
    ...
    public function save_warning($error_message)
    {
        ...
        return $this->update_column_single(
            ['syncing_warnings' => $serialized_warnings],
            ['id' => 1]
        );
    }
    ...
    {
        return $this->update_column_single(
            ['finished_webhooks_deletions' => $status],
            ['id' => 1]
        );
    }
    ...
    {
        return $this->update_column_single(
            ['finished_data_deletions' => $status],
            ['id' => 1]
        );
    }
